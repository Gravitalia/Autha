...
pub fn create_jwt(user_id: String, scope: Vec<String>) -> String {
    let private_key = EncodingKey::from_rsa_pem(std::env::var("RSA_PRIVATE_KEY").expect("Missing env `RSA_PRIVATE_KEY`").as_bytes())
...
pub fn get_jwt(token: String) -> Result<TokenData<Claims>> {
    let public_key = DecodingKey::from_rsa_pem(std::env::var("RSA_PUBLIC_KEY").expect("Missing env `RSA_PUBLIC_KEY`").as_bytes())
        .expect("Failed to load public key");

    Ok(decode::<Claims>(&token, &public_key, &Validation::new(Algorithm::RS256))?)
}
