...
pub async fn check_avatar(avatar: Vec<u8>) -> Result<bool> {
    let mut client = TorreClient::connect(
        std::env::var("TORRESIX_URL")
            .unwrap_or_else(|_| "http://[::1]:50051".to_string()),
    )
    .await?;

    let response = client
        .torre_predict(tonic::Request::new(TorreRequest {
            model: 1,
            data: avatar,
        }))
        .await?;
...
pub async fn upload_avatar(avatar: Vec<u8>) -> Result<String> {
    let mut client = SpinozaClient::connect(
        std::env::var("SPINOZA_URL")
            .unwrap_or_else(|_| "http://[::1]:28717".to_string()),
    )
    .await?;

    let response = client
        .upload(tonic::Request::new(UploadRequest {
            data: avatar,
            width: 256,
            height: 0,
        }))
