...
pub async fn check_turnstile(token: String) -> Result<bool> {
    ...
    data.insert("secret", std::env::var("TURNSTILE_SECRET").expect("Missing env `TURNSTILE_SECRET`"));
...
pub async fn delete_account(url: String, vanity: String) -> Result<bool> {
    let auth_header_value = HeaderValue::from_str(&std::env::var("GLOBAL_AUTH").expect("Missing env `GLOBAL_AUTH`"))?;
