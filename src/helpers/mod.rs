    ...
    tokio::task::spawn(async move {
        loop {
            ...
            if let Ok(x) = crate::database::scylla::query(Arc::clone(&scylla), format!("SELECT vanity FROM accounts.users WHERE expire_at >= '{}' ALLOW FILTERING", now.format("%Y-%m-%d+0000")), []).await {
                ...
                for acc in res.iter() {
                        let _ = crate::database::scylla::query(
                            Arc::clone(&scylla),
                            "UPDATE accounts.users SET email = '', password = '', phone = '', birthdate = '', avatar = '', bio = '', banner = '', mfa_code = '', username = '' WHERE vanity = ?",
                        vec![
                                acc.columns[0].as_ref().unwrap().as_text().unwrap()
                            ]
                        ).await;
                }
            };
