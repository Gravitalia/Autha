...
pub fn init() {
    let _ = SESSION.set(new_session(&ClusterTcpConfig(vec![NodeTcpConfigBuilder::new(dotenv::var("CASSANDRA_HOST").unwrap_or("127.0.0.1:9042".to_string()).as_str(), NoneAuthenticator {}).build()]), RoundRobin::new()).expect("session should be created"));
}
