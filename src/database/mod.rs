...
pub fn get_user(vanity: String) -> Result<crate::model::user::User, String> {
    match mem::get(vanity.clone()).unwrap() {
        ...
        None => {
            let mut cassandra = match cassandra::query("SELECT username, avatar, bio, deleted, flags, verified FROM accounts.users WHERE vanity = ?", vec![vanity.clone()]) {
            ...
            if cassandra.is_empty() {
                cassandra = match cassandra::query("SELECT username, avatar, bio, deleted, flags FROM accounts.bots WHERE id = ?", vec![vanity.clone()]) {
                    Ok(x) => x.get_body().unwrap().as_cols().unwrap().rows_content.clone(),
                    Err(_) => {
                        return Err("Cassandra error".to_string());
                    }
