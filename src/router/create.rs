...
lazy_static! {
    ...
    pub static ref PHONE: Regex = Regex::new(r"^\s*(?:\+?(\d{1,3}))?[-. (]*(\d{3})[-. )]*(\d{3})[-. ]*(\d{4})(?: *x(\d+))?\s*$").unwrap();
    pub static ref BIRTH: Regex = Regex::new(r"^\d{4}-(0[1-9]|1[0-2])-(0[1-9]|[12][0-9]|3[01])$").unwrap();
}
...
pub async fn create(
    ...
    match create_user(
        ...
        hashed_email,
        data.username,
        helpers::crypto::hash(data.password.as_bytes()),
        phone,
        birth,
    )
    .await
