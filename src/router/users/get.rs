...
pub async fn get(scylla: Arc<scylla::Session>, memcached: memcache::Client, id: String, token: Option<String>) -> WithStatus<Json> {
    // Check authorization
    let requester: String;
    let vanity = if id == "@me" && token.is_some() && crate::router::TOKEN.is_match(token.as_deref().unwrap_or_default()) {
        let oauth = match helpers::jwt::get_jwt(token.unwrap_or_default()) {
            Ok(d) => {
                if d.claims.exp <= std::time::SystemTime::now().duration_since(std::time::UNIX_EPOCH).unwrap_or_default().as_secs() {
        ...
        let middelware_res = crate::router::middleware(Arc::clone(&scylla), token, &id)
            .await
        ...
        if middelware_res != "Invalid" && middelware_res != "Suspended" {
            ...
            requester = if id == "@me" || id == vanity { vanity.clone() } else { "".to_string() };
    ...
    // Get user
    let (from_mem, user) = match get_user(scylla, memcached.clone(), vanity.clone(), requester.clone()).await {
        Ok(d) => d,
        Err(e) => {
    ...
    if user.vanity.is_empty() {
        ...
        if !from_mem && vanity != requester {
            let _ = set(memcached, vanity, SetValue::Characters(serde_json::to_string(&user).unwrap_or_default()));
        }
